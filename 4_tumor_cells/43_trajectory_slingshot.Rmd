...
sample_info = readRDS(file = paste0(out_dir, "/../1_metadata/patients_sample_info.rds"))
...
traj_umap = "mnn_39_umap"    # just of visualization
seed = 1337L
traj_max_dims = 39           # to infer trajectory on
```
...
root_cell_id = "GSM5404032_CAACCAATCCCGAGAC-1"
...
We visualize all trajectories :

```{r see_trajectories, fig.width = 12, fig.height = 30}
pdt_df = slingshot::slingPseudotime(my_traj) %>%
  as.data.frame()

plot_list = lapply(colnames(pdt_df), FUN = function(one_pdt) {
  sobj$pseudotime = pdt_df[, one_pdt]
  
  p = Seurat::FeaturePlot(sobj, features = "pseudotime",
                          reduction = traj_umap) +
    ggplot2::scale_color_gradientn(colors = viridis::viridis(n = 100)) +
    ggplot2::labs(title = one_pdt) +
    ggplot2::theme(aspect.ratio = 1,
                   plot.title = element_text(hjust = 0.5)) +
    Seurat::NoAxes()
  
  p2 = ggplot2::ggplot(sobj@meta.data[, c("sample_identifier", "pseudotime")],
                       aes(x = pseudotime,
                           col = sample_identifier,
